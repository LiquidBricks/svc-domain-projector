...
import { validatePayload } from '../../../../../../core/domain/edge/has_task_state/result_computed/validatePayload.js'
...
test('task result validation accepts matching provided statuses', () => {
...
test('task validation rejects conflicting status fields', () => {
  assert.throws(
...
test('task validation rejects missing and unsupported statuses', () => {
...
test('task result validation requires a result and rejects failure payloads', () => {
  assert.throws(
    () => validate({ status: 'provided', stateEdgeStatus: 'provided' }),
    makeDiagnosticsInstance().DiagnosticError,
  )
  assert.throws(
    () => validate({ status: 'error', stateEdgeStatus: 'error', error: { name: 'Error', message: 'failed' } }),
    makeDiagnosticsInstance().DiagnosticError,
  )
  assert.throws(
    () => validate({ result: true, status: 'provided', stateEdgeStatus: 'provided', error: { name: 'Error', message: 'failed' } }),
    makeDiagnosticsInstance().DiagnosticError,
  )
})
